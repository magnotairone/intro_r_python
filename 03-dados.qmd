...
No exemplo abaixo, estamos transformando os dados de `table2` para um formato mais largo, onde cada valor único da variável `type` se torna uma nova coluna. Note que cada unidade de informação (país, ano, casos e contagem) está quebrado em duas linhas. Então essa operação então deixa a tabela de dados mais larga, garantindo que cada unidade de dado esteja representada em uma única linha.
...
Qual mudança foi feita no código do exemplo anterior para constrir os dados usados na geração deste gráfico?
:::
