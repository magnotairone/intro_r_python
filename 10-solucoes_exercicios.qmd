...
Observação: A função `factor()` cria categorias.
`levels()` mostra as categorias únicas.
`table()` ou `summary()` contam as frequências de cada categoria.
:::
...
Este exercício integra todos os conceitos anteriores.
`tapply()` aplica uma função a grupos.
`aggregate()` é alternativa mais clara para operações por grupo.
Este é um exemplo prático de análise de dados que mestrandos em economia encontrarão frequentemente.
:::
